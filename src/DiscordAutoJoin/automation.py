...
try:
    ...
    # Fallback for environments where the async_api submodule may not be statically resolved.
    from playwright import async_api as _playwright_async_api

    async_playwright = _playwright_async_api.async_playwright
    PlaywrightTimeoutError = _playwright_async_api.TimeoutError  # type: ignore[misc]
    PlaywrightError = _playwright_async_api.Error  # type: ignore[misc]
    ...
    async with async_playwright() as p:
        while not state.should_exit.is_set():
            ...
            try:
                ...
                max_launch_retries: int = CONFIG.get("MAX_LAUNCH_RETRIES", 5)
                for attempt in range(1, max_launch_retries + 1):
                    try:
                        ...
                        launch_exc = e
                        log.warn(
                            f"Launch attempt {attempt}/{max_launch_retries} failed: {type(e).__name__}",
                            category="ERR",
                            silent=True,
                        )
                        if context:
                            try:
                                ...
                                logger.debug(
                                    f"Context close during retry failed: {close_err}"
                                )
                        await kill_stale_chrome()
                        remove_chrome_locks()
                ...
                if launch_exc:
                    raise Exception(
                        f"Failed to launch browser after {max_launch_retries} attempts. Last error: {launch_exc}"
                    )

                lower_chrome_priority()

                assert context is not None  # nosec B101
                ...
                await page.goto(
                    CONFIG["DISCORD_URL"], wait_until="domcontentloaded", timeout=60000
                )
                ...
                if (
                    await page.locator(
                        'input[name="email"], [class*="authBox"]'
                    ).count()
                    > 0
                ):
                    ...
                    while (
                        not state.first_run_done.is_set()
                        and not state.should_exit.is_set()
                ...
                for attempt in range(1, CONFIG["MAX_JOIN_RETRIES"] + 1):
                    ...
                    try:
                        btn = page.locator(
                            'button:has-text("Join Voice"), [role="button"][aria-label*="join" i], button[class*="joinBtn"]'
                        ).first
                ...
                # Hardware Toggles
                try:
                    await page.locator('[aria-label="Turn On Camera"]').first.click(
                        timeout=3000
                    )
                except Exception as e:
                ...
                while not state.should_exit.is_set():
                    ...
                    if ui is None:
                        ...
                        if errs >= CONFIG["MAX_CONSECUTIVE_ERRS"]:
                            try:
                                log.warn(
                                    "Page unresponsive. Reloading...", category="SYS"
                                )
                                await page.reload(
                                ...
                                if reload_fails >= CONFIG["MAX_RELOAD_FAILS"]:
                                    log.error(
                                        "Failed to reload page. Restarting context...",
                                        category="ERR",
                                    )
                                    break
                        continue
                    ...
                    if poll % CONFIG["HEALTH_LOG_EVERY"] == 0:
                        log.info(
                            f"Health Check - Voice: {'Drop' if ui['joinVisible'] else 'OK'} | Cam: {'Off' if ui['camOff'] else 'On'} | Mic: {'Unmuted' if ui['micUnmuted'] else 'Muted'}",
                            category="SYS",
                            silent=True,
                        )
                ...
                if (
                    "Target page, context or browser has been closed" not in err_msg
                    and "Browser closed" not in err_msg
                ):
                ...
                if not state.should_exit.is_set() and not state.force_reconnect:
                    ...
                    delay = min(
                        CONFIG["RESTART_DELAY"] * (2 ** (state.restart_count - 1)), 300
                    )
